...
const Particles = {
    ...
    spawn() {
        ...
        for (let i = 0; i < this.count; i++) {
            this.particles.push({
                ...
                // Flow: base angle + speed + phase for organic wandering
                angle: Math.random() * Math.PI * 2,
                speed: 0.3 + Math.random() * 0.5,
                phase: Math.random() * Math.PI * 2,
                wanderStrength: 0.002 + Math.random() * 0.004,
                r: Math.random() * 2.2 + 1,
    ...
    update() {
        ...
        for (const p of particles) {
            ...
            // Add global flow current (slow drift across screen)
            const flowX = Math.sin(time * 0.4 + p.y * 0.003) * 0.25;
            const flowY = Math.cos(time * 0.35 + p.x * 0.003) * 0.25;
            p.vx += flowX;
            p.vy += flowY;
            ...
            if (!this.mouseOnCard && mouse.x > -9000) {
                ...
                if (dist < 250 && dist > 0) {
                    // Stronger pull when closer — creates a cluster effect
                    const force = (250 - dist) / 250;
                    const pull = force * force * 0.06;
                    p.vx += dx / dist * pull;
                    p.vy += dy / dist * pull;
                }
            }
            ...
            // Wrap around edges (smooth wrapping)
            const margin = 30;
            ...
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (dist < connectDist) {
                    ...
                    if (mouseActive) {
                        ...
                        const nearMouse = 1 - Math.min(di, dj) / 250;
                        if (nearMouse > 0) alpha = Math.max(alpha, nearMouse * 0.4);
                    }
